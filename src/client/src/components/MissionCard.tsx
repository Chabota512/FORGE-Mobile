...
export function MissionCard({ mission }: MissionCardProps) {
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      const response = await fetch(`/api/missions/${mission.id}/report/pdf`);
      ...
      a.href = url;
      a.download = `mission-report-${mission.id}.pdf`;
      ...
      toast({
        title: "Success",
        description: "Your mission report is ready.",
      });
    } catch (error: any) {
      toast({
                  ...
                  <div className="bg-black/30 rounded-lg p-3 border border-white/5">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="h-7 text-[10px] font-mono border-primary/30 hover:bg-primary/10 text-primary"
