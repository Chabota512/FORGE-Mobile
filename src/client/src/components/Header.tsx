...
export default function Header({ showBack = true, currentActivity = null }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    setSyncStatus(status.isSyncing ? 'syncing' : 'idle');
    ...
    window.addEventListener('forge-sync-complete', handleSyncComplete);
    return () => window.removeEventListener('forge-sync-complete', handleSyncComplete);
  }, []);
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/90 backdrop-blur-sm">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center text-xs font-mono text-muted-foreground gap-4">
          <span className={`flex items-center gap-2 ${isOnline ? 'text-primary' : 'text-red-500'}`}>
            {isOnline ? (
              <>
